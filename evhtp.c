...
/*! Initialize parser by direct call with buffer
 * Variant of htp__connection_readcb_ for direct call without libevent buffers
 * see htp__connection_readcb_
 * see also evhtp_connection_new_server
 */
int
connection_parse_nobev(char  *buf,
		       size_t buflen,
		       void  *arg)
{
    evhtp_connection_t * c = arg;
...
/* Specialized version of htp__accept_cb_ for clixon
 * see also connection_parse_nobev
 */
evhtp_connection_t *
evhtp_connection_new_server(evhtp_t * htp, evutil_socket_t sock)
{
    evhtp_connection_t * conn;
    conn = htp__connection_new_(htp, sock, evhtp_type_server);
    conn->evbase = htp->evbase;

    if (htp__connection_accept_(htp->evbase, conn) == -1) {
        evhtp_safe_free(conn, evhtp_connection_free);
        return NULL;
    }
